...
public:
    ...
    // ZEUS LEGACY CONSENSUS: coinstake marks its first output as empty.
    void SetEmpty()
    {
        nValue = 0;
        scriptPubKey.clear();
 ...
 * - uint32_t version
 * - uint32_t nTime (Zeus legacy consensus field when version < 2)
...
void UnserializeTransaction(TxType& tx, Stream& s, const TransactionSerParams& params)
{
    ...
    s >> tx.version;
    // ZEUS LEGACY CONSENSUS: transaction versions below 2 serialize nTime
...
void SerializeTransaction(const TxType& tx, Stream& s, const TransactionSerParams& params)
{
    ...
    s << tx.version;
    // ZEUS LEGACY CONSENSUS: preserve historical version<2 transaction timestamp.
    if (tx.version < 2) {
        s << tx.nTime;
    }
    unsigned char flags = 0;
...
public:
    ...
    // ZEUS LEGACY CONSENSUS: existing Zeus transactions default to version 1,
    ...
    // ZEUS LEGACY CONSENSUS: serialized for transaction version < 2.
...
public:
    ...
    // ZEUS LEGACY CONSENSUS: coinstake is identified by a non-coinbase input
