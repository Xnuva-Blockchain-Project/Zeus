 ...
 * A UTXO entry.
 *
 * Zeus migration chainstate format:
 ...
 * The modern Zeus client must rebuild/reindex chainstate from the existing
 * Zeus block history rather than trying to reinterpret an old Bitcoin-style
 * UTXO database.
 */
class Coin
{
public:
    ...
    //! ZEUS LEGACY STAKE METADATA: transaction timestamp used by the stake kernel.
    uint32_t nTime{0};
    ...
    //! Zeus-aware constructors used while connecting real Zeus transactions.
