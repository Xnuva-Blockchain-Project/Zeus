...
#if defined(NDEBUG)
# error "Zeus cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Zeus Signed Message:\n";
...
{
    // Zeus published maximum supply: 40,000,000 ZUS.
    //
    // PoW already ends at consensus.nLastPOWBlock.
    // End new PoS issuance at the same height so that no
    // block after that point can create additional ZUS.
    //
    // Transaction fees remain spendable/reclaimable and
    // therefore are not new monetary issuance.
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Zeus also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    // Zeus also requires low S in sigs
    ...
    if (block.IsProofOfStake() &&
        (pindex->nHeight > chainparams.GetConsensus().nLastPOWBlock ||
         chainparams.GetConsensus().IsProtocolV3(block.GetBlockTime()))) {
...
bool SignBlock(CBlock& block, CWallet& wallet, int64_t& nFees)
{
    ...
    {
        int64_t nStakePastTimeLimit = 0;
        {
            LOCK(cs_main);

            BlockMap::const_iterator miPrev =
                mapBlockIndex.find(block.hashPrevBlock);

            if (miPrev == mapBlockIndex.end() || miPrev->second == NULL)
                return error("SignBlock(): previous block not found");

            nStakePastTimeLimit = miPrev->second->GetPastTimeLimit();
        }

        if (wallet.CreateCoinStake(wallet, block.nBits, 1, block.hashPrevBlock, nFees, txCoinStake, key))
        {
            if (txCoinStake.nTime >= nStakePastTimeLimit + 1)
            {
