...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, const uint256& hashPrevBlock, CAmount& nFees, CMutableTransaction& tx, CKey& key)
{
    // ZUS 40M CAP: anchor staking to the actual block-template parent.
    CBlockIndex* pindexPrev = NULL;
    {
        ...
        if (miPrev == mapBlockIndex.end() || miPrev->second == NULL)
            return error("CreateCoinStake: template previous block not found");
    ...
    // Zeus: Donate to dev fund (or not)
    if (nDonationPercentage > 0) {
        ...
        // Calculate reward
        {
            int64_t nReward = nFees + GetProofOfStakeSubsidy(nHeight);
            if (nReward < 0)
                return false;

            nDevCredit = (GetProofOfStakeSubsidy(nHeight) * nDonationPercentage) / 100;
